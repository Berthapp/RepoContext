...
public static class AgentInstructions
{
    ...
    private static string BuildPlaybook() =>
        string.Join('\n',
            ...
            string.Empty,
            "### Every file type is indexed",
            string.Empty,
            "`repoctx outline <file>` works on source in any language - Python, Go, Java, Rust,",
            "shell and the rest are read by line patterns where no grammar is bundled - and on",
            "specifications, tickets, API contracts, `.feature` files, XML, CSV and settings.",
            "Check a file's skeleton before reading it, whatever it is. `repoctx related <file>`",
            "lists the documents that describe a source file. An empty outline means the format",
            "has no structure to show, not that the file is missing from the index.",
            string.Empty,
            ...
            "- If your client announces a RepoContext session name at the start of a",
            "  conversation, use that one. It belongs to the current context: after a",
            "  compaction, a resume or a fork a new name is announced, and the old one simply",
            "  delivers the evidence again instead of wrongly assuming you still hold it.",
            string.Empty,
            "### If a large read is redirected",
            string.Empty,
            "This repository may have the opt-in read-cost guard installed. A request to read",
            "a large indexed file can then be denied with the cheaper call to make instead.",
            "That is a cost policy, not a permission boundary, and it is bounded:",
            string.Empty,
            "1. Run the suggested `repoctx outline <file>` or",
            "   `repoctx context \"<task>\" --path <file> --detail slices` first; it usually",
            "   answers the question outright.",
            "2. If you genuinely need the whole file, issue the same read again - the repeat",
            "   is allowed through.",
            string.Empty,
            "Nothing else changes: the guard never denies a `repoctx` call, never touches",
            "files the index does not carry, and `repoctx guard status` shows what it did.",
            string.Empty,
            "### Calibration",
            string.Empty,
