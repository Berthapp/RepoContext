...
public static class AgentInstructions
{
    ...
    private static string BuildBlock()
    {
        ...
        return string.Join('\n',
            ...
            string.Empty,
            "Start with one budgeted context call and escalate only on a concrete gap:",
            string.Empty,
            "1. Optional orientation for a new, unfamiliar repository: `repoctx prime`. Use it",
            "   only when your client can keep the primer behind a cache breakpoint; otherwise",
            "   skip this call. It is byte-identical for unchanged indexed content and token calibration.",
            "2. `repoctx context \"<task>\" --detail slices --response-budget-tokens 2000 --format md`",
            "   — ranked source spans packed into a hard response ceiling. Each span carries",
            "   exact lines, reasons and a `receipt`. Markdown avoids JSON escaping for embedded",
            "   code; use JSON when a client must parse the envelope. Use `--detail outline`",
            "   to survey more files, `--detail paths` for locations, or `--strip-comments`",
            "   to drop comment banners (lossy; line ranges become approximate).",
            "3. Only if a file you need is missing: `repoctx search \"<term>\" --symbols --format json`.",
            "4. Only if a relevant symbol was not delivered: `repoctx outline <file> --format json`.",
            "5. Only for dependency or impact questions: `repoctx related <file> --format json`.",
            "6. Only for unfamiliar boundaries: `repoctx architecture --depth 1 --format md`.",
            "7. Only after editing: `repoctx changed --patch --format md`; if it reports",
            "   `stale`, run `repoctx index` (fast, incremental) and re-query.",
            "8. If `context` leaves a concrete knowledge gap, try",
            "   `repoctx memory search \"<topic>\" --format json` before re-deriving it.",
            "   `context` already recalls matching memories automatically. After difficult work, store",
            "   only a distilled, non-secret finding with `repoctx memory add \"<insight>\"",
            "   --kind note|decision|constraint --file <path>`; verify entries flagged `stale`.",
